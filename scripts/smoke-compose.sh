...
WORK_DIR="${SMOKE_WORK_DIR:-$(mktemp -d "/tmp/${PROJECT_NAME}.XXXXXX")}"
...
readonly RUST_MULE_PORT UI_PORT UI_TOKEN OPENAI_API_KEY_VALUE MATTERMOST_WEBHOOK_URL_VALUE
...
cleanup() {
  ...
  if [[ "$cleanup_mode" == "failure" ]]; then
    ...
    if [[ "$KEEP_WORK_DIR_ON_FAILURE" != "true" ]]; then
      rm -rf "$WORK_DIR"
    fi
  else
    if [[ "$KEEP_WORK_DIR_ON_SUCCESS" == "true" ]]; then
...
RUST_MULE_TOKEN_PATH=/data/api.token
...
main() {
  ...
  wait_for_nonempty_file "$DATA_DIR/api.token" "$TIMEOUT_SECS" "rust-mule api token"
